...
<head>
  ...
  <title>AKKWA | List of Fish</title>
                          ...
                          <div
                            ...
                            <div class="card card-rounded">
                              <div class="card-body">
                                ...
                                <h4 class="card-title">List of Recorded Fish</h4>
                                <div class="table-responsive">
                                  <table id="listOfFishTable" class="table table-striped">
                                    <thead>
                                      <tr>
...
<script>
  function loadHistory() {
    fetch("get-readings/get_fish_list.php")
      .then(response => response.json())
      .then(data => {
        let historyTable = document.getElementById("listOfFish");
        historyTable.innerHTML = "";

        data.forEach(row => {

          let date = new Date(row.added_date);
          let options = {
            year: "numeric",
            month: "long",
            day: "numeric",
          ...
          historyTable.innerHTML += `<tr><td>${row.breed}</td><td>${row.type}</td><td>${row.quantity}</td><td>${row.life_span}</td><td>${row.behaviour}</td><td>${formattedDate}</td></tr>`;
        });
        // Initialize DataTable after data is loaded
        if ($.fn.DataTable.isDataTable('#listOfFishTable')) {
          $('#listOfFishTable').DataTable().destroy();
        }

        $('#listOfFishTable').DataTable({
          responsive: true
        });
      })
