...
<head>
  ...
  <title>AKKWA | Add Fish Section</title>
          ...
          <li class="nav-item">
            ...
            <div class="collapse" id="fish">
              <ul class="nav flex-column sub-menu">
                <li class="nav-item">
                  <a class="nav-link" href="#" data-bs-toggle="modal" data-bs-target="#addFishModal">
                    Add Fish
                  </a>
                </li>
              </ul>
                        ...
                        <div class="row flex-grow">
                          <div
                            ...
                            <div class="card card-rounded">
                              <div class="card-body">
                                <h4 class="card-title">Fish Information</h4>
                                <form id="fishForm" class="forms-sample material-form needs-validation" action="save_fish_info.php" method="POST" novalidate>
                                  <div class="form-group">

                                    <select id="breedOfFish" name="breedOfFish" class="js-example-basic-single w-100" required>
                                      <label for="breedOfFish" class="control-label">Breed of Fish</label>
                                  ...
                                  <div class="form-group">
                                    ...
                                    <select id="typeOfFish" name="typeOfFish" class="js-example-basic-single w-100" required>
                                      <label for="typeOfFish" class="control-label">Type of Fish</label>
                                      <option value="" disabled selected>Select Type</option>
                                    </select>
                                    <div class="invalid-feedback">Please select a Type of Fish.</div>
                                  </div>
                                  <div class="form-group">
                                    <input type="text" id="lifeSpan" name="lifeSpan" required="required" />
                                    <label for="input" class="control-label">Life Span</label><i class="bar"></i>
                                    <div class="invalid-feedback">Life Span is required.</div>
                                  </div>
                                  <div class="form-group">
                                    <input type="number" id="quantity" name="quantity" required="required" />
                                    <label for="input" class="control-label">Quantity</label><i class="bar"></i>
                                    <div class="invalid-feedback">Quantity is required.</div>
                                  </div>
                                  <div class="form-group">
                                    <input type="text" id="behavior" name="behavior" required="required" />
                                    <label for="input" class="control-label">Behavior</label><i class="bar"></i>
                                    <div class="invalid-feedback">Behavior is required.</div>
                                  </div>
                                  <div class="form-group">
                                    <input type="date" id="acquisitionDate" name="acquisitionDate" required="required" />
                                    <label for="input" class="control-label">Acquisition Date</label><i class="bar"></i>
                                    <div class="invalid-feedback">Acquisition Date is required.</div>
                                  </div>
                                  <div class="button-container">
                                    <button type="submit" class="btn btn-primary btn-lg" id="saveEvacuee" name="save">
                                      Save Info
                                    </button>
                                  </div>
                                </form>
...
<script>
  const fishTypes = {
    Small: ["Molly", "Platy", "Guppy", "Betta", "Goldfish", "Oscar Cichlid", "Flowerhorn Cichlid", "Tetra", "Danio", "Corydoras Catfish"],
    Medium: ["Clownfish", "Damselfish", "Wrasse", "Butterflyfish", "Angelfish", "Lionfish", "Goby", "Blenny", "Tang", "Moorish Idol"]
  };

  document.getElementById("breedOfFish").addEventListener("change", function() {
    let breed = this.value; // Get selected breed
    let typeOfFish = document.getElementById("typeOfFish");

    // Clear previous options
    typeOfFish.innerHTML = '<option value="" disabled selected>Select Type</option>';

    // Add new options based on breed selection
    if (fishTypes[breed]) {
      fishTypes[breed].forEach(fish => {
        let option = document.createElement("option");
        option.value = fish;
        option.textContent = fish;
        typeOfFish.appendChild(option);
      });
    }

    // Enable the second dropdown after selection
    typeOfFish.disabled = false;
  });
  document.getElementById('fishForm').addEventListener('submit', function(e) {
                                    ...
                                    // Prepare form data with proper error handling
                                    try {
                                      ...
                                      fetch('save_fish_info.php', {
                                        ...
                                        .then(data => {
                                          if (data.status === 'success') {
                                            Swal.fire({
                                              icon: 'success',
                                              title: 'Successfully Added Fish Information!',
                                              showConfirmButton: false,
                                              timer: 1500
                                            }).then(() => {
                                              this.reset();
                                              window.location.href = "add_fish.php";
                                            });
                                          } else {
                                        ...
                                        .catch(error => {
                                          Swal.fire({
                                            ...
                                            text: error.message || 'Unable to submit the form. Please check your internet connection or try again later.',
                                            confirmButtonColor: '#3085d6'
                                          });
                                        });
                                    } catch (error) {
