...
<head>
  ...
  <title>AKKWA | Feeding Control</title>
          ...
          <div class="row" style="margin-top:30px">
            <div class="col-lg-8 col-md-12">
              <div class="d-flex flex-wrap align-items-center gap-2">
                <button type="submit" class="btn btn-primary" id="spinButton">Feed Now</button>
                <label for="timeInput" class="mb-0">Set Food Timer:</label>
                <input type="time" class="form-control w-auto" id="timeInput">
                <button type="submit" class="btn btn-primary" id="setTimeButton">Set Schedule</button>
              </div>
            </div>

            <div class="col-lg-4 col-md-12 d-flex justify-content-lg-end justify-content-md-center mt-3 mt-lg-0">
              <button type="button" class="btn btn-primary btn-rounded btn-icon" data-bs-toggle="modal" data-bs-target="#exportModal" title="Export" >
                <i> Export to PDF </i>
          ...
          <div class="modal fade" id="exportModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <form id="generateReportForm" action="export-to-pdf/feeding_control_report.php" method="POST">
                  ...
                  <div class="modal-body">
                    ...
                    <div class="mb-3">
                      <label for="toDate" class="form-label">Feeding Options</label>
                      <select id="description" name="description" class="js-example-basic-single w-100" required>
                        <option value="" disabled selected>Select </option>
                        <option value="Manual Spin">Manual Spin</option>
                        <option value="Scheduled Spin">Scheduled Spin</option>
                      </select>
                    </div>
                  </div>
                          ...
                          <div
                            ...
                            <div class="card card-rounded">
                              <div class="card-body">
                                ...
                                <h4 class="card-title">Feeding Controls </h4>
                                <div class="table-responsive">
                                  <table id="feedingControl" class="table table-striped">
                                    <thead>
                                      <tr>
                                        <th>Description</th>
                                        <th>Time</th>
...
<script>
  document.getElementById("spinButton").addEventListener("click", function() {
    fetch("esp32_api.php?action=spin")
  ...
  document.getElementById("setTimeButton").addEventListener("click", function() {
    ...
    if (timeValue) {
      ...
      fetch(`esp32_api.php?action=set_time&hour=${hour}&minute=${minute}`)
        .then(response => response.text())
        .then(data => {
          alert(data);
          loadHistory();
  ...
  function loadScheduledTime() {
    fetch("esp32_api.php?action=get_time")
      .then(response => response.json())
      .then(data => {
        if (data.hour !== "00" || data.minute !== "00") { // Default value check
          document.getElementById('timeInput').value = `${data.hour}:${data.minute}`;
        }
      })
      .catch(error => console.error("Error fetching scheduled time:", error));
  }

  window.onload = loadScheduledTime;

  function loadHistory() {
    fetch("get-readings/get_feed_history.php")
      .then(response => response.json())
      .then(data => {
        ...
        data.forEach(row => {
          ...
          historyTable.innerHTML += `<tr><td>${row.description}</td><td>${formattedDate}</td></tr>`;
        });
        // Initialize DataTable after data is loaded
        if ($.fn.DataTable.isDataTable('#feedingControl')) {
          $('#feedingControl').DataTable().destroy();
        }

        $('#feedingControl').DataTable({
          responsive: true
        });
      })
      .catch(error => console.error("Error:", error));
